...
packages = ['basicsr', 'facexlib', 'gfpgan', 'numpy', 'opencv-python', 'Pillow', 'torch', 'torchvision', 'tqdm']
...
def save_image(image_file):
    if image_file is not None:
        ...
        with open(os.path.join(input_folder, filename), "wb") as f:
            ...
            st.success(f"Succesfully uploaded file for processing: {filename}")
...
# Running the model and upscaling user input
if st.button('Enhance'):
    filesinputlist = []
    for filename in os.listdir('Real-ESRGAN/inputs/'):
        if not os.path.isfile(os.path.join('Real-ESRGAN/results/', f"{filename.lower()}_out.")):
            os.system(f"python Real-ESRGAN/inference_realesrgan.py -n RealESRGAN_x4plus -i Real-ESRGAN/inputs/{filename} -o Real-ESRGAN/results --outscale {scale} {half} {face}")
        else:
            # Skip file if it already exists in output (unless specified not to)
            continue
